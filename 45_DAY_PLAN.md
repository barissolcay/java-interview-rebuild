...
Java 25, Temmuz 2026 itibarıyla en güncel LTS sürümüdür. Fakat Java 25'e özgü yeni ve deneysel özellikleri öğrenmeye çalışmayacaksın. İş görüşmeleri için Java 17+ seviyesindeki temel dil, collections, streams, OOP ve concurrency bilgisi daha önemlidir. 

Spring'in resmi sitesi şu anda Spring Boot 4.1.0'ı gösteriyor. Spring Initializr'ın önerdiği kararlı sürümü seç ve 45 gün boyunca sürüm değiştirme. 
...
Spring Academy'nin REST kursu yaklaşık beş saatlik ve uçtan uca API, güvenlik ve test içeriyor. Bunu bir günde izlemeyeceksin; programa dağıtılmış şekilde kullanacaksın. 

PostgreSQL'in güncel resmi tutorial'ı ilişkisel veritabanı ve SQL için yeterli bir referans sunuyor. Docker'ın Java rehberi de Spring Boot uygulamasını container'a alma, veritabanı bağlama ve container içinde test çalıştırmayı kapsıyor. 

# AI kullanım protokolün

2025 JetBrains araştırmasında geliştiricilerin büyük çoğunluğu AI araçlarını kullandığını bildiriyor. Buna karşılık Stack Overflow araştırmasında profesyonel projelerde API kalitesi ve genel yazılım kalitesi, sırf "AI entegrasyonu yapmaktan" daha öncelikli görülüyor. Yani AI bilmek önemli; fakat hatalı, test edilmemiş ve açıklayamadığın kod üretmek değerli değil. 
...
Her gün `AI_USAGE.md` dosyasına şunları ekle:
...
4. `LEARNING_LOG.md` güncellenecek.
...
Oracle'ın Collections dokümantasyonunu yalnızca takıldığın API'leri kontrol etmek için kullan. 
...
`JOB_SKILLS.md` oluştur ve 10 Java/new-grad ilanında geçen becerileri işaretlemek için şu başlıkları ekle:
...
Oracle'ın stream dokümanında source, intermediate operation ve terminal operation kavramlarını kontrol et. 
...
Beş güncel ilan seç ve `JOB_SKILLS.md` dosyasına ekle. İlanlarda geçen ortak ifadeleri kendi cümlelerinle yaz.
...
Flyway `V1__initial_schema.sql` oluştur.
...
PostgreSQL resmi tutorial'ındaki SQL giriş bölümünü referans olarak kullan. 
...
Hata yönetiminin güvenlik açısından da önemli olduğunu OWASP dokümanı vurguluyor.
...
Son kural için `external_reference` unique constraint düşün.
...
Testcontainers, gerçek PostgreSQL örneğini geçici container'da başlatarak repository ve SQL davranışını gerçek veritabanıyla test etmeye yarar. 
...
Spring Security classpath'e eklendiğinde web uygulamasını varsayılan olarak güvenli hâle getirir; kendi production ayarını ayrıca yapılandırman gerekir.
...
Bu, object-level authorization problemidir. OWASP, kullanıcıdan alınan bir ID ile veri kaynağına erişen her endpoint'te nesne seviyesinde yetki kontrolü yapılmasını öneriyor. 
...
Docker'ın Java rehberini referans al. 
...
Spring Boot production özellikleri arasında metrics, health checks ve externalized configuration bulunuyor. 
...
GitHub'ın resmi Maven rehberi localde kullandığın Maven komutlarının CI ortamında build ve test için çalıştırılmasını gösteriyor. 
...
- `challenge.status`
- `challenge.end_at`
- `progress_entry.challenge_id`
- `progress_entry.user_id`
...
OWASP'ın LLM risklerinde prompt injection ve excessive agency öne çıkıyor; agentic sistemlerde tool misuse ve data leakage ayrıca önemli. 
